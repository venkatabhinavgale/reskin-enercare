...
<div class="block-offer-card__wrapper" data-allow-multiple>
  <div class="block-offer-card__container <?= $classes ?>">
    ...
    <?php if ($campaign_expiration && $campaign_expiration != "") { ?><p class="block-offer-card__offer-expiration">Offer expires <?= date('F d, Y', strtotime($campaign_expiration)); ?>.<?php } ?><button class="block-offer-card__terms-toggle" aria-controls="terms_<?= $campaign->ID ?>" data-micromodal-trigger="modal-<?=$campaign->ID ?>" aria-label="Open modal and view details and fine print for <?= $campaign_heading ?>">View Details</button></p>
